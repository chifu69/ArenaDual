...
function drawEntity(p){ctx.save();ctx.shadowBlur=26;ctx.shadowColor=p.color;ctx.fillStyle=p.color;ctx.beginPath();ctx.arc(p.x,p.y,p.r,0,Math.PI*2);ctx.fill();ctx.shadowBlur=0;ctx.strokeStyle='#dff7ff';ctx.lineWidth=4;ctx.stroke();ctx.restore();}
